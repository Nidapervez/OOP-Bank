...
class Bank {
    customer = [];
    account = [];
    addcustomer(obj) { this.customer.push(obj); }
    addaccountnum(obj) { this.account.push(obj); }
    transcation(accObj) {
        let NewAccounts = this.account.filter((асс) => acc.accNumber !== accObj.accNumber);
...
async function bankService(bank) {
    do {
        ...
        console.log(chalk.red.bold("insufficient balance"), let, newBalance = account.balance - ans.rupee, bank.transcation({ accNumber: account.accNumber, balance: newBalance }));
    } while ();
